...
const Header = ({ 
  ...
  onProfileClick, onHomeClick, isProfileActive 
}) => {
  ...
  const btnClass = "p-2 rounded-xl border-2 border-ink dark:border-chalk hover:bg-black/5 dark:hover:bg-white/10 active:scale-95 transition-all";
    ...
    <div className="flex flex-col gap-6 mb-8">
      ...
      {!isProfileActive && (
        <div className="bg-paper-card dark:bg-night-card border-3 border-ink dark:border-chalk rounded-3xl p-4 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex items-center gap-4 md:gap-8 w-full md:w-auto justify-between md:justify-start">
                <button onClick={handlePrevMonth} className={btnClass}><ChevronLeft size={24} /></button>
                <h2 className="text-2xl md:text-3xl font-black tracking-tight text-ink dark:text-chalk w-32 text-center select-none">
                    {format(currentDate, 'MMM')}
                </h2>
                <button onClick={handleNextMonth} className={btnClass}><ChevronRight size={24} /></button>
            </div>
      ...
      {isProfileActive && (
         <div className="bg-paper-card dark:bg-night-card border-3 border-ink dark:border-chalk rounded-3xl p-4 shadow-sm animate-in fade-in slide-in-from-bottom-2">
            <h2 className="text-2xl font-black text-center text-ink dark:text-chalk">Your Profile</h2>
         </div>
      )}
    </div>
  );
